...
def CLEANING(OPEN,CLOSE, HIGH, LOW):              #主力清洗
    ...
    INTO = IF(VAR5 > REF(VAR5, 1), VAR5, 0)
    CLEAN =IF(VAR5 < REF(VAR5, 1), VAR5, 0)
    VAR21 = SMA(ABS(HIGH - VAR1), 13, 1) / SMA(MIN(HIGH - VAR1, 0), 10, 1)
    VAR31 = EMA(VAR21, 10)
    VAR41 = HHV(HIGH, 33)
    VAR51 = EMA(IF(HIGH >= VAR41, VAR31, 0), 3)
    PULLUP = IF(VAR51 < REF(VAR51, 1), VAR51, 0)
    SHIPPING = IF(VAR51 > REF(VAR51, 1), VAR51, 0)
    return RD(INTO), RD(CLEAN), RD(PULLUP), RD(SHIPPING)